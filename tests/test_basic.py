...
from dataclass_cli import build_config_from_cli, cli_help, cli_exclude, cli_file_loadable
...
class TestBasicFunctionality:
    ...
    def test_simple_config_creation(self):
        ...
        config = build_config_from_cli(SimpleConfig, [
            '--name', 'test',
            '--count', '5',
            '--debug', 'true'
        ])
    ...
    def test_complex_types(self):
        ...
        config = build_config_from_cli(ComplexConfig, [
            '--host', 'localhost',
            '--port', '9000',
            '--items', 'item1',
            '--items', 'item2',
    ...
    def test_optional_types(self):
        ...
        config1 = build_config_from_cli(ComplexConfig, [
            '--host', 'localhost'
        ])
        assert config1.timeout is None
        
        # With optional value
        config2 = build_config_from_cli(ComplexConfig, [
            '--host', 'localhost',
            '--timeout', '15.0'
        ])
...
class TestErrorHandling:
    ...
    def test_invalid_dataclass(self):
        ...
        with pytest.raises(ConfigBuilderError, match="config_class must be a dataclass"):
    ...
    def test_invalid_type_conversion(self):
        ...
        with pytest.raises(SystemExit):  # argparse exits on invalid type
            build_config_from_cli(SimpleConfig, [
                '--name', 'test',
                '--count', 'not-a-number'
            ])
...
class TestAnnotations:
    ...
    def test_cli_exclude_annotation(self):
        ...
        config = build_config_from_cli(self.AnnotatedConfig, [
            '--public-field', 'test',
            '--another-field', '100'
        ])
...
class TestEdgeCases:
    ...
    def test_all_optional_config(self):
        ...
        # With values
        config2 = build_config_from_cli(self.AllOptionalConfig, [
            '--field1', 'test',
            '--field2', '42'
        ])
