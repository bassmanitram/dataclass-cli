...
from dataclass_cli.file_loading import load_file_content, process_file_loadable_value, is_file_loadable_value
...
class TestFileLoadableConfig:
    ...
    def test_literal_values(self):
        ...
        config = build_config_from_cli(FileLoadableConfig, [
            '--name', 'TestApp',
            '--system-prompt', 'You are a test assistant',
            '--welcome-message', 'Welcome to the test!'
        ])
    ...
    def test_file_loading(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as system_file:
            system_content = "You are an expert assistant with deep knowledge."
            system_file.write(system_content)
            system_file.flush()
            
            with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as welcome_file:
                ...
                try:
                    config = build_config_from_cli(FileLoadableConfig, [
                        '--name', 'FileApp',
                        '--system-prompt', f'@{system_file.name}',
                        '--welcome-message', f'@{welcome_file.name}'
                    ])
    ...
    def test_mixed_literal_and_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            ...
            try:
                config = build_config_from_cli(FileLoadableConfig, [
                    '--name', 'MixedApp',
                    '--system-prompt', 'Literal prompt text',  # Literal
                    '--welcome-message', f'@{f.name}'  # File
                ])
    ...
    def test_default_values_with_file_loadable(self):
        ...
        config = build_config_from_cli(FileLoadableConfig, [
            '--name', 'DefaultApp'
            # system_prompt should use default, welcome_message should be None
        ])
        
        assert config.name == 'DefaultApp'
        assert config.system_prompt == 'You are helpful'  # Default value
        assert hasattr(config, 'welcome_message')  # Field exists but may be None
    
    def test_file_loading_error_handling(self):
        """Test error handling when file loading fails."""
        with pytest.raises(SystemExit):  # CLI parsing should fail gracefully
            # This will cause a configuration error during build_config
            try:
                build_config_from_cli(FileLoadableConfig, [
                    '--name', 'ErrorApp',
                    '--system-prompt', '@/nonexistent/file.txt'
                ])
            except SystemExit:
                raise  # Re-raise SystemExit
            except Exception as e:
                # If we get a different exception, that's also acceptable
                assert "Failed to process field" in str(e) or "File not found" in str(e)
