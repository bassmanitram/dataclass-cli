...
from typing import Any, Dict, List, Optional, Type, Union, get_type_hints, get_origin, get_args, Set, Callable
from dataclasses import fields, is_dataclass

from .annotations import is_cli_excluded, get_cli_help, is_cli_file_loadable
...
class GenericConfigBuilder:
    ...
    def __init__(
        ...
        if not is_dataclass(config_class):
            raise ConfigBuilderError(f"config_class must be a dataclass, got {config_class}")
        
        if exclude_fields and include_fields:
            raise ConfigBuilderError("Cannot specify both exclude_fields and include_fields")
    ...
    def _should_include_field(self, field_name: str, field_info: Dict[str, Any]) -> bool:
    ...
    def _analyze_config_fields(self) -> Dict[str, Dict[str, Any]]:
        ...
        for field_obj in fields(self.config_class):
            ...
            field_info = {
                ...
                'is_list': is_list,
                'is_dict': is_dict,
                'default': field_obj.default if field_obj.default is not field_obj.default_factory else None,
    ...
    def _add_field_argument(self, parser: argparse.ArgumentParser, field_name: str, info: Dict[str, Any]) -> None:
        ...
        if info['is_list']:
            ...
            parser.add_argument(
                cli_name,
                type=str,
                help=f'{help_text} configuration file path'
            )
    ...
    def build_config(self, args: argparse.Namespace, base_config_name: str = 'config') -> Any:
        """
        ...
        if base_config_value:
            try:
                ...
                raise ConfigurationError(f"Failed to load base config from {base_config_value}: {e}") from e
        ...
        try:
            ...
            raise ConfigurationError(f"Failed to create {self.config_class.__name__}: {e}") from e
    
    def _merge_cli_args(self, base_config: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
        ...
        for field_name, info in self._config_fields.items():
            ...
            if cli_value is not None:
                if info['is_list']:
                    ...
                    try:
                        ...
                        raise ConfigurationError(f"Failed to load dictionary config for field '{field_name}' from {cli_value}: {e}") from e
                else:
                    # Simple override - check for file-loadable fields
                    try:
                        processed_value = process_file_loadable_value(cli_value, field_name, info)
                        config[field_name] = processed_value
                    except (ValueError, Exception) as e:
                        raise ConfigurationError(f"Failed to process field '{field_name}': {e}") from e
            ...
            if info['is_dict'] and override_value:
                ...
                try:
                    ...
                    raise ConfigurationError(f"Failed to apply property overrides for field '{field_name}': {e}") from e
        
        return config
    
    def _apply_property_overrides(self, target_dict: Dict[str, Any], overrides: List[str]) -> None:
    ...
    def _set_nested_property(self, target: Dict[str, Any], path: str, value: Any) -> None:
...
def build_config_from_cli(
    ...
    parser = argparse.ArgumentParser(description=f'Build {config_class.__name__} from CLI')
